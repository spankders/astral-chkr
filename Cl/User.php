...
class Cl_User
{
	...
	public function registration( array $data )
	{
		if( !empty( $data ) ){
			...
			$query = "INSERT INTO users (user_id, name, email, password, key_asc, level, creditos, created) VALUES (NULL, '$name', '$email', '$password', '$key_asc' '$level' '$creditos' CURRENT_TIMESTAMP)";
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "SELECT user_id, name, email, key_asc, creditos, level FROM users where ( email = '$email' and key_asc = '$key_asc' ) and password = '$password'";
	...
	public function forgetPassword( array $data )
	{
		if( !empty( $data ) ){
			...
			if(mysqli_query($this->_con, $query)){
				...
				$headers = "From: support@astralchecker.com" . "\r\n" .
						"CC: support@astralchecker.com";
